      ...
      <div className="document-paper">
        ...
        <div className="doc-content">
          {data.content ? data.content.split('\n').map((line, i) => (
            ...
            <>
              1. 구정 발전과 구민 안전을 위해 헌신하시는 노고에 깊이 감사드립니다.<br/>
              2. 관내 도로 모니터링 시스템(AI 스마트 관제)을 통해 아래와 같이 <strong>도로 내 위험 요소[{translateType(data.event_type||data.type)}]</strong>가 감지되었음을 보고합니다.<br/>
        ...
        <div className="doc-images" style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginTop: "40px"}}>
          { data.image_url || (data.detection_images && data.detection_images.length > 0) || (data.images && data.images.length > 0) ? (
            <div className="doc-img-box" style={{
              backgroundImage: `url(${getImageUrl(data.image_url || (data.detection_images && data.detection_images[0]?.image_path) || (data.images && data.images[0]?.url) || (data.images && data.images[0]?.image_path))})`,
